...
调度器在北京时间每天上午 8:00 查询当天课表，然后按照每门课程的开课时间注册定时器；它不会每分钟重复登录和查询整张课表。当天无课程时会直接结束当天任务，不会继续查询。课表查询失败时默认最多尝试 3 次，每次间隔 5 分钟；达到上限后跳过当天，次日 8:00 再开始。签到窗口从开课前 10 分钟开始。单次签到失败时每隔 30 秒继续尝试，直到接口确认签到成功；签到重试仍不会因为达到固定次数而停止。

课表查询上限可以通过环境变量调整：

```bash
AUTO_SIGN_SCHEDULE_MAX_ATTEMPTS=3
AUTO_SIGN_SCHEDULE_RETRY_DELAY_MS=300000
```

如果查询接口返回 `429 请求过于频繁`，调度器会读取响应中的 `Retry-After`，并至少等待 5 分钟后再尝试；不会在限流期间快速连续请求。
